...
force_values = 7;
Omega_values = [0, 1*pi/20, 3*pi/20, pi/4, pi/2];

% Simulation states
states = [
    % struct('J', 1, 'K', 1, 'id', 'StaticSync')
    % struct('J', 1, 'K', -1, 'id', 'StaticAsync')
...
for i = 1:length(states)
    for F_ext = force_values
        for Omega_ext = Omega_values
            simulation_id = sprintf('%s_Force%d_Omega%.2f', states(i).id, F_ext, Omega_ext);
